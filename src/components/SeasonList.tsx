...
const SeasonList = ({
  ...
  const [currentExternalLink, setCurrentExternalLink] = useState<string>('');
  const [currentExternalType, setCurrentExternalType] = useState<string>('');
  ...
  const handleExternalPlayer = async (link: string, type: string) => {
    ...
    setCurrentExternalLink(link);
    setCurrentExternalType(type);
    ...
    try {
      ...
      availableStream.forEach((s, i) => console.log(`Server ${i + 1}:`, s.server, s.type));
      ...
      ToastAndroid.show(`Found ${availableStream.length} servers`, ToastAndroid.SHORT);
    } catch (error) {
              ...
              <>
                ...
                <ScrollView style={{ maxHeight: 300 }}>
                  ...
                  {externalPlayerStreams.length === 0 && (
                    <Text className="text-white text-center p-4">No servers available</Text>
                  )}
                </ScrollView>
